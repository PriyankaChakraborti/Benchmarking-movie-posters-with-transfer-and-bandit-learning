...
def make_prediction(image_uploaded,img_shape):
    ...
    if start_int == 0:
        ...
        # Below needed as there is a bug
        tf.compat.v1.experimental.output_all_intermediates(True)
        
        with graph.as_default():
            model=load_model('static/data/vgg16_1.h5')
...
def make_recommendations(class_layer,bandit_arm, df_rec, feature_array):
...
def resultspage():
    ...
    layer=make_prediction(target_name,img_shape)
    ...
    found,not_found,filenames=grab_movie_posters(make_recommendations(layer,arm_name, df_rec, feature_array))
